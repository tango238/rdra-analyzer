...
{{
  "policies": [
    ...
    def _derive_scenarios(
        ...
        # UC.category（= BUCの所属業務名）でグルーピング
        category_to_ucs: dict[str, list[Usecase]] = {}
        for uc in usecases:
            if uc.category:
                category_to_ucs.setdefault(uc.category, []).append(uc)

        # BUC名 → UCを探すヘルパー（UC名の部分一致でフォールバック）
        def _find_uc_for_buc(buc_name: str) -> Usecase | None:
            # 1. category（業務名）完全一致
            for cat, ucs in category_to_ucs.items():
                if cat in buc_name or buc_name in cat:
                    return ucs[0]
            # 2. UC名とBUC名の部分一致
            for uc in usecases:
                if uc.name in buc_name or buc_name in uc.name:
                    return uc
            # 3. フォールバック
            return usecases[0] if usecases else None

        sc_counter = 0

        for flow in flows:
            target_uc = _find_uc_for_buc(flow.buc_name)
            if not target_uc:
                continue
        ...
        for scene in scenes:
            target_uc = _find_uc_for_buc(scene.buc_name)
            if not target_uc:
                continue
