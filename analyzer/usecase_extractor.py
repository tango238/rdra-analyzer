...
def _api_path_segments(route_or_call: str) -> list[str]:
    """`"<METHOD> <path>"` / `"<path>"` からパスのセグメント列を取り出す。

    先頭スラッシュ・クエリ・末尾スラッシュは無視する。
    """
    parts = route_or_call.split(" ", 1)
...
def _api_paths_match(a: list[str], b: list[str]) -> bool:
    """API パスのセグメント列がサフィックス一致するか（プレースホルダ対応）。

    フロントの api_calls は `/api/v1` 等のプレフィックスを省くことがあるため、
    短い方が長い方の末尾サフィックスに一致すれば同一エンドポイントとみなす。
    1セグメントだけのサフィックス過剰一致は防ぐ。
    """
    n = min(len(a), len(b))
    if n == 0:
        return False
    if n < 2 and len(a) != len(b):
        return False
    return all(_seg_eq(x, y) for x, y in zip(a[-n:], b[-n:]))
...
"""
    ...
    def _enrich_pages(
        ...
        """related_routes と pages の api_calls を突き合わせて
        関連フロントページ(route_path)とビュー(component)を紐付ける。

        related_views だけでなく related_pages(route_path) も埋める。
        related_pages は loop-e2e の navigate 照合・reconcile のキーになるため。
        既存(LLM由来)の related_pages は温存して和集合を取る。
        """
        if not pages:
            return

        for uc in usecases:
            views: set[str] = set(uc.related_views)  # 既存(LLM/前回)を温存
            related_pages: set[str] = set(uc.related_pages)  # LLM由来を温存
            uc_keys = [_api_path_segments(r) for r in uc.related_routes]

            for page in pages:
                # ページの api_calls がユースケースのルートと一致するか
                # （プレフィックス差・プレースホルダを許容したサフィックス一致）
                for api_call in page.api_calls:
                    call_key = _api_path_segments(api_call)
                    if any(_api_paths_match(call_key, k) for k in uc_keys):
                        ...
                        views.add(label)
                        if page.route_path:
                            related_pages.add(page.route_path)
                        break

            uc.related_views = sorted(views)
            uc.related_pages = sorted(related_pages)
