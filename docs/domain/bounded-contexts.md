...
### ④ 可視化コンテキスト (Supporting) — **別パッケージ `@rdra/viewer` として分離（discover 確定）**

- **責務**: 抽出・合成された RDRA モデルを Mermaid 図・インタラクティブビューワーで PdM に見せる。説明の器。
- **デプロイ境界**: 同一コードベース内 BC から **独立 npm パッケージ CLI `@rdra/viewer`** へ分離。
  Core（Python）が view-model JSON を吐き、`@rdra/viewer`（Node）がそれだけを入力に描画・配信（dumb renderer）。
  → 1 BC = 1 デプロイ単位に整合。集計/レイアウトロジックは現状維持（[discovery.md](discovery.md)「ビューワー切り出し方針」）。
- **主要概念**: Mermaid 図, ビューワー, クロスリファレンス, コード参照箇所, **view-model（表示契約）**
- **ユビキタス言語（表示固有）**: ER図 / 複合図 / パネル / タブ / ズーム・パン / クロスリファレンス。
  ※これらは**描画専用の語彙**であり、ドメイン語ではない。
- **言語境界の事実（重要）**: ④ は **Core のユビキタス言語をそのまま話す Conformist な読み取り射影**。
  `usecases` / `entities` / `scenarios` / `state_machines` / `policies` 等の語を**翻訳せず**受け取る。
  → **独自モデルを持たないため ACL 不要**。これは ② 実績調停（loop-e2e の外部言語を `normalize_route` で翻訳＝ACL あり）との決定的な違い。
- **所有チーム**: TBD（分離後も Core と同一オーナー想定 ＝ 公開 API ではなく内部契約）
...
- **可視化 BC は「翻訳しない」**：④ 可視化は Core の語（usecases/entities/…）を**そのまま**描画する Conformist 射影で、独自言語を持たない＝**ACL 不要**。対して ② 実績調停は loop-e2e の**外部言語**を内部語へ翻訳する＝**ACL あり**。「外部システムとの境界か／内部モデルの読み取り射影か」で ACL 要否が分かれる。`@rdra/viewer` 分離後もこの非対称は保たれる。
