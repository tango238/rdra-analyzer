...
| **3** | `scenarios` | 操作シナリオ生成（**非推奨**: loop-e2e に一本化。`--force` で従来動作） | JSON |
| **4** | `rdra` | RDRAモデル生成（情報モデル・ユースケース複合図・アクティビティ図） | Mermaid/Markdown |
| **5** | `viewer` | インタラクティブビューワー起動 | HTML |
| - | `enrich` | UCの `related_*` を checkpoint から再導出（LLM不要） | JSON |
| - | `reconcile` | loop-e2e のシナリオ（`loop-e2e-pending.json`）を取り込む | JSON |
...
#### enrich: ユースケースの関連情報を補完

`analyze` 由来の各ユースケースの `related_controllers` / `related_views` / `related_pages` を
`_checkpoint.json`（routes / pages）から決定的に埋め直す（LLM呼び出し不要）。

```bash
python main.py enrich
```

- `related_controllers`: `related_routes` → コントローラーを紐付け。
- `related_views` / `related_pages`: ページの `api_calls` と `related_routes` を、`/api/v1` などの
  プレフィックス差・`:id` / `{id}` プレースホルダを許容したサフィックス一致で突き合わせて紐付け。
- `related_pages` は loop-e2e の navigate 照合・`reconcile` の紐付けキーになるため、これが埋まると
  loop-e2e のシナリオが pending ではなく既存ユースケースへ直接紐付くようになる。

> 古い生成物の補完や、`reconcile` で新規UCを足した後のリフレッシュに使う。`rdra` 実行時にも
> 同じ紐付けが自動適用される。

#### reconcile: loop-e2e シナリオの取り込み
